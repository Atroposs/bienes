...
GRUPO_AUDITOR = "auditor"
GRUPO_ENCARGADO = "encargado"
...
def es_auditor(user):
    ...
    return user.is_superuser or pertenece_a_grupo(user, GRUPO_AUDITOR)

def es_encargado(user):
    if not user.is_authenticated:
        return False
    return pertenece_a_grupo(user, GRUPO_ENCARGADO)
